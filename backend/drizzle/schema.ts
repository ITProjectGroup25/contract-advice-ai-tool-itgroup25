...
export const formDetails = pgTable(
  "form_details",
  {
...
export const question = pgTable(
  "question",
  {
    ...
    formId: bigint("form_id", { mode: "number" }).notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    sectionId: bigint("section_id", { mode: "number" }).references(() => formSection.id, {
      onDelete: "set null",
    }),
...
export const person = pgTable(
  "person",
  {
    ...
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    orgUnitId: bigint("org_unit_id", { mode: "number" }).references(() => orgUnit.id, {
      onDelete: "set null",
    }),
...
export const submission = pgTable(
  ...
  (table) => {
    return {
      ...
      idxSubmissionTypeTime: index("idx_submission_type_time").on(
        table.createdAt,
        table.workflowType
      ),
...
export const attachment = pgTable(
  "attachment",
  {
    ...
    submissionId: bigint("submission_id", { mode: "number" })
      ...
      .references(() => submission.id, { onDelete: "cascade" }),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    questionId: bigint("question_id", { mode: "number" }).references(() => question.id, {
      onDelete: "set null",
    }),
...
export const guidanceBinding = pgTable(
  "guidance_binding",
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint("id", { mode: "number" }).primaryKey().notNull(),
    ...
    resourceId: bigint("resource_id", { mode: "number" })
      .notNull()
      .references(() => guidanceResource.id, { onDelete: "cascade" }),
...
export const guidanceTrigger = pgTable(
  "guidance_trigger",
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint("id", { mode: "number" }).primaryKey().notNull(),
...
export const guidanceSearchLog = pgTable(
  "guidance_search_log",
  {
    ...
    id: bigint("id", { mode: "number" }).primaryKey().notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    submissionId: bigint("submission_id", { mode: "number" }).references(() => submission.id, {
      onDelete: "set null",
    }),
    queryText: varchar("query_text", { length: 500 }).notNull(),
    resultsCount: integer("results_count"),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    clickedResourceId: bigint("clicked_resource_id", { mode: "number" }).references(
      () => guidanceResource.id,
      { onDelete: "set null" }
    ),
    createdAt: timestamp("created_at", { mode: "string" }).defaultNow().notNull(),
...
export const submissionDraftAnswer = pgTable(
  "submission_draft_answer",
  {
    ...
    draftId: bigint("draft_id", { mode: "number" })
      .notNull()
      .references(() => submissionDraft.id, { onDelete: "cascade" }),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    questionId: bigint("question_id", { mode: "number" })
      .notNull()
      .references(() => question.id, { onDelete: "restrict" }),
...
export const exportJob = pgTable(
  ...
  (table) => {
    return {
      ...
      idxExportFilter: index("idx_export_filter").on(
        table.dateFrom,
        table.dateTo,
        table.formId,
        table.workflowType
...
export const adminUserRole = pgTable(
  ...
  (table) => {
    return {
      adminUserRolePkey: primaryKey({
        columns: [table.adminUserId, table.roleName],
...
export const formDefaultRecipient = pgTable(
  ...
  (table) => {
    return {
      formDefaultRecipientPkey: primaryKey({
        columns: [table.formId, table.email],
        name: "form_default_recipient_pkey",
      }),
    };
  }
);
...
export const answerMultiValue = pgTable(
  ...
  (table) => {
    return {
      answerMultiValuePkey: primaryKey({
        columns: [table.answerId, table.itemValue],
        name: "answer_multi_value_pkey",
      }),
    };
  }
);
...
export const emailAttachmentMap = pgTable(
  ...
  (table) => {
    return {
      emailAttachmentMapPkey: primaryKey({
...
export const guidanceResourceTag = pgTable(
  "guidance_resource_tag",
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    resourceId: bigint("resource_id", { mode: "number" })
      .notNull()
  ...
  (table) => {
    return {
      guidanceResourceTagPkey: primaryKey({
        columns: [table.resourceId, table.tag],
        name: "guidance_resource_tag_pkey",
      }),
    };
  }
);
...
export const submissionDraftAnswerMulti = pgTable(
  ...
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    draftAnswerId: bigint("draft_answer_id", { mode: "number" })
      .notNull()
      .references(() => submissionDraftAnswer.id, { onDelete: "cascade" }),
    itemValue: varchar("item_value", { length: 255 }).notNull(),
  },
  (table) => {
    return {
      submissionDraftAnswerMultiPkey: primaryKey({
...
export const validationRuleParam = pgTable(
  "validation_rule_param",
  {
    ...
    ruleId: bigint("rule_id", { mode: "number" })
      .notNull()
      .references(() => validationRule.id, { onDelete: "cascade" }),
    paramKey: varchar("param_key", { length: 50 }).notNull(),
    paramValue: varchar("param_value", { length: 255 }).notNull(),
  },
  (table) => {
    return {
      validationRuleParamPkey: primaryKey({
        columns: [table.ruleId, table.paramKey],
        name: "validation_rule_param_pkey",
      }),
    };
  }
);
