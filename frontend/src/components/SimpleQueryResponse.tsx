...
import { Bot, ArrowLeft, ThumbsUp, ThumbsDown, CheckCircle, AlertCircle } from "lucide-react";
import { useState, useEffect } from "react";
import { localDB } from "../utils/localDatabase";
import exampleImage from '../assets/6c4e5b3173e6796addc153b43f2a18f19d22d312.png';
...
export function SimpleQueryResponse({ onBack, onSatisfied, onNeedHumanHelp, submissionId }: SimpleQueryResponseProps) {
  ...
  // Sample response based on common simple query types
  const generateResponse = () => {
    return `Thank you for your simple query submission. Based on the information provided:
...
This automated response covers approximately 80% of simple queries. If your specific situation requires personalized attention, our grants team will review your submission within 1-2 business days.`;
  ...
  const handleSatisfied = async () => {
    // Update database with user satisfaction
    if (submissionId) {
      try {
  ...
  const handleNeedHelp = async () => {
    // Update database with escalation request
    if (submissionId) {
      try {
        await localDB.updateSubmission(submissionId, {
          userSatisfied: false,
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            Your simple query has been processed
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Automated guidance based on your query
          </CardDescription>
        </CardHeader>
