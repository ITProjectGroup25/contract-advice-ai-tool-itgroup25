...
class EmailService {
      ...
      const checkEmailJS = () => {
        ...
        if (attempts >= maxAttempts) {
          ...
          console.log(
            ...
            Object.keys(window).filter((key) => key.toLowerCase().includes("email"))
          );
  ...
  async sendGrantTeamNotification(emailData: GrantTeamEmailData): Promise<boolean> {
    ...
    try {
      ...
      if (!config.serviceId || !config.publicKey) {
        throw new Error(
          "Email configuration is incomplete. Please check Service ID and Public Key."
        );
      }
      ...
      const response = await emailjsLib.send(config.serviceId, templateId, templateParams);
    ...
    notification.innerHTML = `
      ...
      <div style="opacity: 0.9;">
        ...
        <div style="margin-top: 8px; padding: 8px; background: rgba(255,255,255,0.1); border-radius: 4px; font-size: 12px;">
          ...
          ${
            hasCustomError
              ? "<br><br><em>Note: Email service is temporarily unavailable.</em>"
  ...
  async sendConfirmationEmail(emailData: EmailData, config?: EmailConfig): Promise<boolean> {
    ...
    try {
      ...
      if (!activeConfig.serviceId || !activeConfig.templateId || !activeConfig.publicKey) {
        throw new Error(
          "Email configuration is incomplete. Please check Service ID, Template ID, and Public Key."
        );
      }
      ...
      try {
        ...
        this.showDemoEmailNotification(
          emailData,
          "EmailJS library could not be loaded. This may be due to network restrictions or CDN issues."
        );
        return true;
      }
      ...
      if (activeConfig.templateId === getCurrentGrantTeamTemplateId()) {
        console.error(
          "❌ CRITICAL ERROR: User confirmation email is using grant team template ID!"
        );
        console.error("User Template ID:", activeConfig.templateId);
        console.error("Grant Team Template ID:", getCurrentGrantTeamTemplateId());
        throw new Error(
          "Template ID conflict: User confirmation email cannot use grant team template"
        );
      }
  ...
  private showDemoEmailNotification(emailData: EmailData, customError?: string): void {
    ...
    const headerText = hasCustomError
      ? "EmailJS Loading Error"
      : isConfigInvalid
    ...
    notification.innerHTML = `
      ...
      <div style="opacity: 0.9;">
        ...
        <div style="margin-top: 8px; padding: 8px; background: rgba(255,255,255,0.1); border-radius: 4px; font-size: 12px;">
          ...
          ${
            hasCustomError
              ? "<br><br><em>Note: Email service is temporarily unavailable. Please try again later.</em>"
              : isConfigInvalid
                ? "<br><br><em>Note: This is a demo. Configure EmailJS in Admin Panel to send real emails.</em>"
...
export {
  ...
  savePublicKey,
  getCurrentConfig,
  getCurrentGrantTeamEmail,
  saveGrantTeamEmail,
  getCurrentGrantTeamTemplateId,
  saveGrantTeamTemplateId,
